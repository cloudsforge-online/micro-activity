...
lifecycle
  ...
  .addProbe(
    ...
    //
    // It matters more here than in most services: `POST /ingest` is authenticated by a signature
    // as well as a token, so a feed that stops ingesting because identity blinked would be a feed
    // that falls behind for a reason unrelated to anything it does.
...
//    the pool so ingest is real rather than a lazily-connected surprise on the first delivery.
