 ...
 *
 * ══════════════════════════════════════════════════════════════════════════════════════════════
 * **WAVE M2: TWO MODULES, TWO MIGRATION LEDGERS, AND WHY THEY CANNOT BE CONFUSED.**
 *
 * This process now migrates activity's databases and notify's. Both ledgers are a table called
 * `schema_migrations` — the name is a literal inside `@cloudsforge/db` and takes no option — so
 * the ONLY thing keeping activity's version 7 from being read as notify's version 7 is that they
 * are in different DATABASES. Nothing about the merge changes that, and nothing may:
 *
 *   * `ACTIVITY_DATABASE_URL` and `NOTIFY_DATABASE_URL` name different databases. `assertDistinct`
 *     below REFUSES to run if they do not, before a single statement is issued. It matters more
 *     here than it did in wave M1: both modules own a table called `inbox` with IDENTICAL columns
 *     and both own a `jobs`, so a shared database would not even fail on a conflicting DDL.
 *   * The `service` name each `migrate()` call passes is distinct (`activity` / `notify`), so the
 ...
 *
 * **THE TARGET COUNT IS ASYMMETRIC AND THAT IS THE DESIGN.** activity has a testnet DATABASE and
 * so has two entries; notify has one, because it carries the network as a COLUMN
 * (`deliveries.network`). See `notify/module.ts`'s selector for the argument.
 *
...
// ONE import, and it returns four scalars and an array of DDL. It deliberately does NOT reach for
// `./notify/env.ts`: that record carries `SMTP_PASS` and `NOTIFY_INGEST_SIGNING_SECRET`, and a
// migrator holding either is a second entry point with a live credential in scope for no reason at
// all. See `notifyMigrationTargets`'s own note, and `moduleboundary.test.ts`.
import { notifyMigrationTargets } from './notify/module.ts'
...
//
// One entry per module per network. activity's testnet half is conditional until its testnet
...
const targets: readonly Target[] = [
  ...
  ...(env.databaseUrlTestnet
    ...
    : []),
  // The notify module names its own, because only it may read its own configuration.
