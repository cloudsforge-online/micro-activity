 ...
 *
 * It is not hypothetical for this pair. activity and notify BOTH own a table called `inbox` and
 * BOTH own a table called `jobs`, and the two `inbox` tables have identical columns — so one shared
 * database would not even fail on a conflicting `create table`.
 *
